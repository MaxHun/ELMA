...
void AnalyzerRadiusOfGyration<IngredientsType>::initialize()
{
	...
	if(groups.size()==0){
        // ab hier nur für Moleküle, deren Tag 1 oder 2 ist:
        for(int idx=1, idx< 100000, idx++){ // find number of molecules instead of 100000
             if((ingredients.getMolecules()[idx].getAttributeTag()==1) || (ingredients.getMolecules()[idx].getAttributeTag()==2)){
		        groups.push_back(MonomerGroup<molecules_type>(ingredients.getMolecules()[idx]));
